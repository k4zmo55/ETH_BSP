...
=============================================================
GELISIM DURUMU: 26 Agustos 2026 - Staj 23. Gun

Bir onceki gune (25 Agustos) ek olarak bugun FaultInjectionWorker
("Hata Enjeksiyonu" sekmesi) ve FirmwareUpdateWorker ("Bootloader (IAP)"
sekmesi) tamamlandi. Boylece arayuzde planlanan yedi sekmenin tumu
calisir hale geldi; bu, E-AETIS GUI'nin gelistirme surecinin tamamlandigi
gundur (bkz. staj_defteri_final.txt, 26 Agustos 2026).

Ethernet_BSP (bare-metal STM32 Ethernet BSP) dogrulama arayuzu.

Protokol: UDP / ASCII istek, "BASLIK|ANAHTAR:DEGER|..." yanit.
Firmware karsiligi: Src/eth_app.c -> eaetis_handle()

Ekran goruntusu almak icin:  python 26_agustos.py
Bagimliliklar:  pip install PyQt5 pyqtgraph
"""
...
class FaultInjectionWorker(QThread):
    """Bozuk paket enjeksiyonu. Amac: cihazin HardFault almadigini dogrulamak."""
    log = pyqtSignal(str)
    progress = pyqtSignal(int)

    def __init__(self, proto, fuzz_count, do_oversize, do_truncated, do_random):
        super().__init__()
        self.proto = proto
        self.fuzz_count = fuzz_count
        self.do_oversize = do_oversize
        self.do_truncated = do_truncated
        self.do_random = do_random

    def _alive(self):
        """Cihaz hala cevap veriyor mu? Kararlilik olcutu budur."""
        data, _ = self.proto.request("GET_PHY_DMA_STATS", timeout=1.5)
        return data is not None

    def run(self):
        self.log.emit("Hata enjeksiyonu basliyor. Once temel kontrol...")
        if not self._alive():
            self.log.emit("Cihaz teste baslamadan once yanit vermiyor. Iptal.")
            return
        self.log.emit("Temel kontrol OK. Cihaz canli.")

        steps = sum([self.do_oversize, self.do_truncated, self.do_random])
        done = 0

        if self.do_oversize:
            self.log.emit("[1] 2048 baytlik dev paket (IP parcalanmasi tetiklenir)...")
            self.proto.fire(b"X" * 2048)
            time.sleep(0.2)
            self.log.emit("     Not: BSP fragment reassembly YAPMAZ. Bu paketin "
                          "sessizce dusurulmesi BEKLENEN dogru davranistir.")
            done += 1
            self.progress.emit(int(done * 100 / max(1, steps)))

        if self.do_truncated:
            self.log.emit("[2] Kesik/tutarsiz basliklar gonderiliyor...")
            malformed = [
                b"START_IAP",                      # eksik SIZE/CRC
                b"START_IAP|SIZE:|CRC:",           # bos alanlar
                b"FW_DATA|SEQ:9999|LEN:99999|",    # tutarsiz uzunluk
                b"PHY_READ|REG:99",                # gecersiz register
                b"PHY_WRITE|REG:1|VAL:ZZZZ",       # gecersiz deger
                b"GET_PHY_DMA_STATS" + b"\x00" * 64,
                b"|" * 200,
                b"\x00",
            ]
            for m in malformed:
        ...
        if self.do_random:
            self.log.emit("[3] %d adet rastgele fuzz paketi..." % self.fuzz_count)
            for i in range(self.fuzz_count):
                size = random.randint(1, 900)
                self.proto.fire(bytes(random.getrandbits(8) for _ in range(size)))
                if i % 20 == 0:
                    time.sleep(0.01)
            done += 1
            self.progress.emit(int(done * 100 / max(1, steps)))
        ...
        time.sleep(0.5)
        self.log.emit("Enjeksiyon bitti. Cihaz canlilik kontrolu...")

        if self._alive():
            self.log.emit("BASARILI: Cihaz hala yanit veriyor. Parser dayanikli, "
                          "HardFault yok.")
            hw = self.proto.get_stats()
            if hw:
                self.log.emit("Dusen paket sayisi: %s | CRC hata: %s"
                              % (hw.get("RX_DROP", "?"), hw.get("CRC_ERR", "?")))
        else:
            self.log.emit("BASARISIZ: Cihaz yanit vermiyor. Muhtemel HardFault "
                          "veya ring tikanmasi. SWD ile fault register'larina bakin.")


class FirmwareUpdateWorker(QThread):
    """Ethernet uzerinden IAP guncelleme."""
    log = pyqtSignal(str)
    progress = pyqtSignal(int)
    ...
    def run(self):
        if not os.path.isfile(self.path):
            self.log.emit(".bin dosyasi bulunamadi: %s" % self.path)
            return
        ...
        self.log.emit("Dosya: %s | %d bayt | CRC32 0x%08X"
                      % (os.path.basename(self.path), size, crc))

        data, _ = self.proto.request("START_IAP|SIZE:%d|CRC:0x%08X" % (size, crc),
                                     timeout=15.0)   # flash silme uzun surer
        if data is None:
            self.log.emit("Cihaz START_IAP'a yanit vermedi. Firmware'de "
                          "ETH_ENABLE_IAP=1 mi?")
            return

        header, kv = self.proto.parse(data)
        if header != "IAP_READY":
            self.log.emit("Cihaz hazir degil: %s" % data.decode("utf-8", "replace"))
            return

        self.log.emit("Cihaz hazir. Flash silindi. Yukleme basliyor...")
        ...
        for i in range(total):
            ...
            packet = ("FW_DATA|SEQ:%d|LEN:%d|" % (i, len(piece))).encode("utf-8") + piece
            ...
            for attempt in range(3):
                ...
                if text.startswith("ACK:%d" % i):
                    ok = True
                    break
                self.log.emit("Paket #%d reddedildi: %s" % (i, text))

            if not ok:
                self.log.emit("Paket #%d yazilamadi. Yukleme iptal edildi." % i)
                return

            self.progress.emit(int((i + 1) * 100 / total))

        self.log.emit("Tum paketler gonderildi. Bitis ve CRC dogrulama...")
        resp, _ = self.proto.request("END_IAP", timeout=10.0)

        if resp is None:
            self.log.emit("Bitis yanitI alinamadi. Cihaz yeni firmware ile "
                          "yeniden baslamis olabilir.")
            return

        text = resp.decode("utf-8", "replace")
        if "FLASH_SUCCESS" in text or "JUMP_OK" in text:
            self.log.emit("BASARILI: Firmware yazildi ve CRC dogrulandi. "
                          "Cihaz yeni uygulamaya atladi.")
        else:
            self.log.emit("BASARISIZ: %s" % text)
...
class EAETISWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.tabs.addTab(self._tab_fault(),     "Hata Enjeksiyonu")
        self.tabs.addTab(self._tab_iap(),       "Bootloader (IAP)")
    ...
    def _tab_fault(self):
        ...
        note = QLabel(
            "Bu test cihazi bilerek bozuk verilerle besler. Basari olcutu "
            "'cihaz reddetti' degil, 'cihaz test SONRASINDA hala yanit veriyor' "
            "olmasidir. 2048 baytlik paket IP katmaninda parcalanir; BSP "
            "reassembly yapmadigi icin bu paketin dusurulmesi beklenen dogru "
            "davranistir.")
        note.setWordWrap(True)
        note.setStyleSheet("color:#555; padding:6px;")
        lay.addWidget(note)

        cfg = QGroupBox("Enjeksiyon Secenekleri")
        g = QGridLayout(cfg)

        self.fi_oversize = QCheckBox("Dev paket (2048 bayt, IP parcalanmasi)")
        self.fi_oversize.setChecked(True)
        g.addWidget(self.fi_oversize, 0, 0)

        self.fi_truncated = QCheckBox("Kesik / tutarsiz protokol basliklari")
        self.fi_truncated.setChecked(True)
        g.addWidget(self.fi_truncated, 1, 0)

        self.fi_random = QCheckBox("Rastgele fuzz verisi")
        self.fi_random.setChecked(True)
        g.addWidget(self.fi_random, 2, 0)

        g.addWidget(QLabel("Fuzz paket sayisi:"), 2, 1)
        self.fi_count = QSpinBox(); self.fi_count.setRange(1, 100000)
        self.fi_count.setValue(200)
        g.addWidget(self.fi_count, 2, 2)

        btn = QPushButton("Enjeksiyonu Baslat")
        btn.clicked.connect(self.do_fault_injection)
        g.addWidget(btn, 3, 0)

        lay.addWidget(cfg)

        self.fi_progress = QProgressBar()
        lay.addWidget(self.fi_progress)
        lay.addStretch()
        return w
    ...
    def _tab_iap(self):
        ...
        warn = QLabel(
            "DIKKAT: Bu islem cihazin flash bellegini siler ve yeniden yazar. "
            "Firmware tarafinda ETH_ENABLE_IAP=1 olmalidir. Kesinti halinde "
            "cihaz SWD ile kurtarilmasi gerekebilir.")
        warn.setWordWrap(True)
        warn.setStyleSheet("background:#fdf3d7; border:1px solid #e0c060; "
                           "padding:10px; border-radius:4px;")
        lay.addWidget(warn)

        row = QHBoxLayout()
        self.iap_path = QLineEdit()
        self.iap_path.setPlaceholderText("Firmware .bin dosyasi secin...")
        row.addWidget(self.iap_path, 1)

        b = QPushButton("Gozat")
        b.clicked.connect(self.do_pick_bin)
        row.addWidget(b)

        self.iap_start = QPushButton("Yuklemeyi Baslat")
        self.iap_start.clicked.connect(self.do_firmware_update)
        row.addWidget(self.iap_start)
        lay.addLayout(row)

        self.iap_info = QLabel("Dosya secilmedi.")
        self.iap_info.setFont(MONO)
        lay.addWidget(self.iap_info)

        self.iap_progress = QProgressBar()
        lay.addWidget(self.iap_progress)
        lay.addStretch()
        return w
    ...
    def do_fault_injection(self):
        self._sync_proto()
        self.fi_progress.setValue(0)
        w = FaultInjectionWorker(self.proto, self.fi_count.value(),
                                 self.fi_oversize.isChecked(),
                                 self.fi_truncated.isChecked(),
                                 self.fi_random.isChecked())
        w.progress.connect(self.fi_progress.setValue)
        self._run(w)

    def do_pick_bin(self):
        path, _ = QFileDialog.getOpenFileName(self, "Firmware sec", "",
                                              "Binary (*.bin);;Tum dosyalar (*)")
        if not path:
            return
        self.iap_path.setText(path)
        with open(path, "rb") as f:
            blob = f.read()
        self.iap_info.setText(
    ...
    def do_firmware_update(self):
        path = self.iap_path.text().strip()
        if not path:
            self.log("Once bir .bin dosyasi secin.")
            return

        answer = QMessageBox.warning(
            self, "Firmware Guncelleme",
            "Cihazin flash bellegi silinecek ve yeniden yazilacak.\n"
            "Devam edilsin mi?",
            QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        if answer != QMessageBox.Yes:
            self.log("Guncelleme iptal edildi.")
            return

        self._sync_proto()
        self.iap_progress.setValue(0)
        w = FirmwareUpdateWorker(self.proto, path)
        w.progress.connect(self.iap_progress.setValue)
        w.finished.connect(lambda: self.iap_start.setEnabled(True))
        self.iap_start.setEnabled(False)
        self._run(w)
